...
def motion_global_anchor_lin_vel_xy_error_exp(env: ManagerBasedRLEnv, command_name: str, std: float) -> torch.Tensor:
    command: MotionCommand = env.command_manager.get_term(command_name)
    error = torch.sum(torch.square(command.anchor_lin_vel_w[:, :2] - command.robot_anchor_lin_vel_w[:, :2]), dim=-1)
    return torch.exp(-error / std**2)

def motion_global_anchor_ang_vel_yaw_error_exp(env: ManagerBasedRLEnv, command_name: str, std: float) -> torch.Tensor:
    command: MotionCommand = env.command_manager.get_term(command_name)
