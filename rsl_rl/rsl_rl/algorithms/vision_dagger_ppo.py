...
from rsl_rl.models import MLPModel, VisionMLPModel
from rsl_rl.storage import RolloutStorage
from rsl_rl.utils import resolve_callable, resolve_obs_groups, resolve_optimizer


class VisionDAggerPPO:
    """DAgger-PPO Algorithm: PPO with an added DAgger (Imitation) Loss term."""

    actor: VisionMLPModel
    """The actor model."""

    critic: MLPModel
    """The critic model."""

    teacher: MLPModel | None = None # [DAgger 改动] 增加 Teacher 模型定义
    ...
    def update(self) -> dict[str, float]:
        ...
        for batch in generator:
            ...
            # [DAgger 改动] 混合动态 Loss 计算
            # L = (1-alpha)*PPO_Loss + alpha*DAgger_Loss
            ppo_loss = surrogate_loss + self.value_loss_coef * value_loss - self.entropy_coef * entropy.mean()
            
            if self.teacher:
                loss = (1.0 - alpha) * ppo_loss + alpha * dagger_loss
                # loss = ppo_loss + alpha * dagger_loss
            else:
                loss = ppo_loss
            ...
            # Compute the gradients for DAggerPPO
    ...
    def load(self, loaded_dict: dict, load_cfg: dict | None, strict: bool) -> bool:
        ...
        if load_cfg.get("actor"):
            ...
            print("[INFO] Vision Actor (CNN + MLP) loaded successfully.")
            print(f"Loading actor keys: {loaded_dict['actor_state_dict'].keys()}")
    ...
    @staticmethod
    def construct_algorithm(obs: TensorDict, env: VecEnv, cfg: dict, device: str) -> VisionDAggerPPO:
        """Construct the DAggerPPO algorithm."""
        # Resolve class callables
        alg_class: type[VisionDAggerPPO] = resolve_callable(cfg["algorithm"].pop("class_name"))  # type: ignore
        
        # [Vision 适配] 解析 Actor 类
        actor_class: type[VisionMLPModel] = resolve_callable(cfg["actor"].pop("class_name"))  # type: ignore
        critic_class: type[MLPModel] = resolve_callable(cfg["critic"].pop("class_name"))  # type: ignore
        # DAgger
        teacher_class: type[MLPModel] = resolve_callable(cfg["teacher"].get("class_name", "MLPModel")) # type: ignore

        # Resolve observation groups
        default_sets = ["actor", "critic", "teacher"] # [DAgger 改动] 加入 teacher 组
        ...
        num_proprio = total_policy_dim - 5046
        cfg["actor"]["num_proprio"] = num_proprio
        print(f"[VisionDAgger] Detected Proprio Dim: {num_proprio}, Depth: 5046")

        # Initialize the policy
        actor: VisionMLPModel = actor_class(obs, cfg["obs_groups"], "actor", env.num_actions, **cfg["actor"]).to(device)
        # from rsl_rl.distributions import Normal
        # init_noise_std = getattr(cfg["actor"], "init_noise_std", 1.0)
        # actor.distribution = Normal(env.num_actions, init_noise_std)
        ...
        # [DAgger 改动] 初始化并加载 Teacher
        teacher_cfg = cfg.pop("teacher") # 提取 teacher 配置
        teacher_model_path = teacher_cfg.pop("model_path", None) # 获取权重路径
        ...
        # [DAgger 改动] 传入 teacher
        alg: VisionDAggerPPO = alg_class(actor, critic, storage, teacher=teacher, device=device, **cfg["algorithm"], multi_gpu_cfg=cfg["multi_gpu"])
        return alg
