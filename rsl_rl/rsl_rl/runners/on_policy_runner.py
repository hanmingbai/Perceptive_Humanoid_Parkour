...
from rsl_rl.algorithms import PPO, DAggerPPO, HeightDAggerPPO, VisionDAggerPPO
...
class OnPolicyRunner:
    ...
    alg: PPO | DAggerPPO | HeightDAggerPPO | VisionDAggerPPO
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device: str = "cpu") -> None:
        ...
        alg_class: type[VisionDAggerPPO] = resolve_callable(self.cfg["algorithm"]["class_name"])  # type: ignore
    ...
    def export_policy_to_onnx(self, path: str, filename: str = "policy.onnx", verbose: bool = False) -> None:
        """导出 ONNX 模型，并针对 Vision 任务自动适配维度。"""
        import os
        if not os.path.exists(path):
            os.makedirs(path)
        
        # 1. 检查当前算法是否为视觉蒸馏类 (或者通过类名字符串判断)
        is_vision = "Vision" in self.alg.__class__.__name__
        
        if is_vision:
            # 获取本体感知维度 (96)
            num_proprio = getattr(self.alg.actor, "num_proprio", 96)
            # 获取深度图展平后的维度 (5046)
            # 也可以通过 obs_groups 动态计算，这里暂定 5046
            total_dim = num_proprio + 5046
            print(f"[ONNX Export] Detected Vision Model. Using total dim: {total_dim}")
            dummy_input = torch.zeros(1, total_dim).to(self.device)
                # 2. 执行导出
            ...
            torch.onnx.export(
                ...
                model_path,
                export_params=True,
                opset_version=14, # 建议使用 14+ 以支持某些复杂的 CNN 算子
                do_constant_folding=True,
                input_names=['input'],
                output_names=['output'],
                dynamic_axes={'input': {0: 'batch_size'}, 'output': {0: 'batch_size'}}
            )
            print(f"[ONNX Export] Policy exported to: {model_path}")
        else:
            onnx_model = self.alg.get_policy().as_onnx(verbose=verbose)
            onnx_model.to("cpu")
            onnx_model.eval()
